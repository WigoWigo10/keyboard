...
- Low level OS requests (directkeys._os_keyboard) are mocked out by rewriting
...
def send_instant_event(event):
    if directkeys._listener.direct_callback(event):
        output_events.append(event)

# Mock out side effects.
directkeys._os_keyboard.init = lambda: None
directkeys._os_keyboard.listen = lambda callback: None
directkeys._os_keyboard.map_name = dummy_keys.__getitem__
directkeys._os_keyboard.press = lambda scan_code: send_instant_event(make_event(KEY_DOWN, None, scan_code))
directkeys._os_keyboard.release = lambda scan_code: send_instant_event(make_event(KEY_UP, None, scan_code))
directkeys._os_keyboard.type_unicode = lambda char: output_events.append(KeyboardEvent(event_type=KEY_DOWN, scan_code=999, name=char))
...
trigger = lambda e=None: directkeys.press(999)
triggered_event = [KeyboardEvent(KEY_DOWN, scan_code=999)]

class TestKeyboard(unittest.TestCase):
    def tearDown(self):
        directkeys.unhook_all()
        #self.assertEquals(directkeys._hooks, {})
        #self.assertEquals(directkeys._hotkeys, {})

    def setUp(self):
        #directkeys._hooks.clear()
        #directkeys._hotkeys.clear()
        del input_events[:]
        del output_events[:]
        directkeys._recording = None
        directkeys._pressed_events.clear()
        directkeys._physically_pressed_keys.clear()
    ...
    def test_is_modifier_name(self):
        for name in directkeys.all_modifiers:
            self.assertTrue(directkeys.is_modifier(name))
    def test_is_modifier_scan_code(self):
        for i in range(10):
            self.assertEqual(directkeys.is_modifier(i), i in [4, 5, 6, 7])
    ...
    def test_key_to_scan_codes_brute(self):
        for name, entries in dummy_keys.items():
            ...
            self.assertEqual(directkeys.key_to_scan_codes(name), expected)
    def test_key_to_scan_code_from_scan_code(self):
        for i in range(10):
            self.assertEqual(directkeys.key_to_scan_codes(i), (i,))
    def test_key_to_scan_code_from_letter(self):
        self.assertEqual(directkeys.key_to_scan_codes('a'), (1,))
        self.assertEqual(directkeys.key_to_scan_codes('A'), (1,-1))
    def test_key_to_scan_code_from_normalized(self):
        self.assertEqual(directkeys.key_to_scan_codes('shift'), (5,6))
        self.assertEqual(directkeys.key_to_scan_codes('SHIFT'), (5,6))
        self.assertEqual(directkeys.key_to_scan_codes('ctrl'), directkeys.key_to_scan_codes('CONTROL'))
    def test_key_to_scan_code_from_sided_modifier(self):
        self.assertEqual(directkeys.key_to_scan_codes('left shift'), (5,))
        self.assertEqual(directkeys.key_to_scan_codes('right shift'), (6,))
    def test_key_to_scan_code_underscores(self):
        self.assertEqual(directkeys.key_to_scan_codes('_'), (12,))
        self.assertEqual(directkeys.key_to_scan_codes('right_shift'), (6,))
    def test_key_to_scan_code_error_none(self):
        with self.assertRaises(ValueError):
            directkeys.key_to_scan_codes(None)
    def test_key_to_scan_code_error_empty(self):
        with self.assertRaises(ValueError):
            directkeys.key_to_scan_codes('')
    def test_key_to_scan_code_error_other(self):
        with self.assertRaises(ValueError):
            directkeys.key_to_scan_codes({})
    def test_key_to_scan_code_list(self):
        self.assertEqual(directkeys.key_to_scan_codes([10, 5, 'a']), (10, 5, 1))
    def test_key_to_scan_code_empty(self):
        with self.assertRaises(ValueError):
            directkeys.key_to_scan_codes('none')
    def test_key_to_scan_code_duplicated(self):
        self.assertEqual(directkeys.key_to_scan_codes('duplicated'), (20,))

    def test_parse_hotkey_simple(self):
        self.assertEqual(directkeys.parse_hotkey('a'), (((1,),),))
        self.assertEqual(directkeys.parse_hotkey('A'), (((1,-1),),))
    def test_parse_hotkey_separators(self):
        self.assertEqual(directkeys.parse_hotkey('+'), directkeys.parse_hotkey('plus'))
        self.assertEqual(directkeys.parse_hotkey(','), directkeys.parse_hotkey('comma'))
    def test_parse_hotkey_keys(self):
        self.assertEqual(directkeys.parse_hotkey('left shift + a'), (((5,), (1,),),))
        self.assertEqual(directkeys.parse_hotkey('left shift+a'), (((5,), (1,),),))
    def test_parse_hotkey_simple_steps(self):
        self.assertEqual(directkeys.parse_hotkey('a,b'), (((1,),),((2,),)))
        self.assertEqual(directkeys.parse_hotkey('a, b'), (((1,),),((2,),)))
    def test_parse_hotkey_steps(self):
        self.assertEqual(directkeys.parse_hotkey('a+b, b+c'), (((1,),(2,)),((2,),(3,))))
    def test_parse_hotkey_example(self):
        alt_codes = directkeys.key_to_scan_codes('alt')
        shift_codes = directkeys.key_to_scan_codes('shift')
        a_codes = directkeys.key_to_scan_codes('a')
        b_codes = directkeys.key_to_scan_codes('b')
        c_codes = directkeys.key_to_scan_codes('c')
        self.assertEqual(directkeys.parse_hotkey("alt+shift+a, alt+b, c"), ((alt_codes, shift_codes, a_codes), (alt_codes, b_codes), (c_codes,)))
    def test_parse_hotkey_list_scan_codes(self):
        self.assertEqual(directkeys.parse_hotkey([1, 2, 3]), (((1,), (2,), (3,)),))
    def test_parse_hotkey_deep_list_scan_codes(self):
        result = directkeys.parse_hotkey('a')
        self.assertEqual(directkeys.parse_hotkey(result), (((1,),),))
    def test_parse_hotkey_list_names(self):
        self.assertEqual(directkeys.parse_hotkey(['a', 'b', 'c']), (((1,), (2,), (3,)),))

    def test_is_pressed_none(self):
        self.assertFalse(directkeys.is_pressed('a'))
    def test_is_pressed_true(self):
        self.do(d_a)
        self.assertTrue(directkeys.is_pressed('a'))
    def test_is_pressed_true_scan_code_true(self):
        self.do(d_a)
        self.assertTrue(directkeys.is_pressed(1))
    def test_is_pressed_true_scan_code_false(self):
        self.do(d_a)
        self.assertFalse(directkeys.is_pressed(2))
    def test_is_pressed_true_scan_code_invalid(self):
        self.do(d_a)
        self.assertFalse(directkeys.is_pressed(-1))
    def test_is_pressed_false(self):
        self.do(d_a+u_a+d_b)
        self.assertFalse(directkeys.is_pressed('a'))
        self.assertTrue(directkeys.is_pressed('b'))
    def test_is_pressed_hotkey_true(self):
        self.do(d_shift+d_a)
        self.assertTrue(directkeys.is_pressed('shift+a'))
    def test_is_pressed_hotkey_false(self):
        self.do(d_shift+d_a+u_a)
        self.assertFalse(directkeys.is_pressed('shift+a'))
    def test_is_pressed_multi_step_fail(self):
        self.do(u_a+d_a)
        with self.assertRaises(ValueError):
            directkeys.is_pressed('a, b')

    def test_send_single_press_release(self):
        directkeys.send('a', do_press=True, do_release=True)
        self.do([], d_a+u_a)
    def test_send_single_press(self):
        directkeys.send('a', do_press=True, do_release=False)
        self.do([], d_a)
    def test_send_single_release(self):
        directkeys.send('a', do_press=False, do_release=True)
        self.do([], u_a)
    def test_send_single_none(self):
        directkeys.send('a', do_press=False, do_release=False)
        self.do([], [])
    def test_press(self):
        directkeys.press('a')
        self.do([], d_a)
    ...
    def test_press_and_release(self):
        directkeys.press_and_release('a')
        self.do([], d_a+u_a)

    def test_send_modifier_press_release(self):
        directkeys.send('ctrl+a', do_press=True, do_release=True)
        self.do([], d_ctrl+d_a+u_a+u_ctrl)
    def test_send_modifiers_release(self):
        directkeys.send('ctrl+shift+a', do_press=False, do_release=True)
    ...
    def test_call_later(self):
        ...
        directkeys.call_later(fn, (1, 2), 0.01)
    ...
    def test_hook_nonblocking(self):
        ...
        def count(e):
            ...
            self.i += 1
        hook = directkeys.hook(count, suppress=False)
        self.do(d_a+u_a, d_a+u_a)
        self.assertEqual(self.i, 2)
        directkeys.unhook(hook)
        self.do(d_a+u_a, d_a+u_a)
        self.assertEqual(self.i, 2)
        directkeys.hook(count, suppress=False)
        self.do(d_a+u_a, d_a+u_a)
    ...
    def test_hook_blocking(self):
        ...
        def count(e):
            ...
            self.i += 1
            return e.name == 'b'
        hook = directkeys.hook(count, suppress=True)
        self.do(d_a+d_b, d_b)
        self.assertEqual(self.i, 2)
        directkeys.unhook(hook)
        self.do(d_a+d_b, d_a+d_b)
        self.assertEqual(self.i, 2)
        directkeys.hook(count, suppress=True)
        self.do(d_a+d_b, d_b)
        self.assertEqual(self.i, 4)
        directkeys.unhook_all()
        self.do(d_a+d_b, d_a+d_b)
    ...
    def test_on_press_nonblocking(self):
        directkeys.on_press(lambda e: self.assertEqual(e.name, 'a') and self.assertEqual(e.event_type, KEY_DOWN))
        self.do(d_a+u_a)
    def test_on_press_blocking(self):
        directkeys.on_press(lambda e: e.scan_code == 1, suppress=True)
        self.do([make_event(KEY_DOWN, 'A', -1)] + d_a, d_a)
    def test_on_release(self):
        directkeys.on_release(lambda e: self.assertEqual(e.name, 'a') and self.assertEqual(e.event_type, KEY_UP))
        self.do(d_a+u_a)

    def test_hook_key_invalid(self):
        with self.assertRaises(ValueError):
            directkeys.hook_key('invalid', lambda e: None)
    def test_hook_key_nonblocking(self):
        self.i = 0
        def count(event):
            self.i += 1
        hook = directkeys.hook_key('A', count)
        self.do(d_a)
    ...
    def test_hook_key_blocking(self):
        ...
        hook = directkeys.hook_key('A', count, suppress=True)
        self.do(d_a, d_a)
        self.assertEqual(self.i, 1)
    ...
    def test_on_press_key_nonblocking(self):
        directkeys.on_press_key('A', lambda e: self.assertEqual(e.name, 'a') and self.assertEqual(e.event_type, KEY_DOWN))
        self.do(d_a+u_a+d_b+u_b)
    def test_on_press_key_blocking(self):
        directkeys.on_press_key('A', lambda e: e.scan_code == 1, suppress=True)
        self.do([make_event(KEY_DOWN, 'A', -1)] + d_a, d_a)
    def test_on_release_key(self):
        directkeys.on_release_key('a', lambda e: self.assertEqual(e.name, 'a') and self.assertEqual(e.event_type, KEY_UP))
        self.do(d_a+u_a)

    def test_block_key(self):
        blocked = directkeys.block_key('a')
        self.do(d_a+d_b, d_b)
        self.do([make_event(KEY_DOWN, 'A', -1)], [make_event(KEY_DOWN, 'A', -1)])
        directkeys.unblock_key(blocked)
        self.do(d_a+d_b, d_a+d_b)
    def test_block_key_ambiguous(self):
        directkeys.block_key('A')
        self.do(d_a+d_b, d_b)
        self.do([make_event(KEY_DOWN, 'A', -1)], [])

    def test_remap_key_simple(self):
        mapped = directkeys.remap_key('a', 'b')
        self.do(d_a+d_c+u_a, d_b+d_c+u_b)
        directkeys.unremap_key(mapped)
        self.do(d_a+d_c+u_a, d_a+d_c+u_a)
    def test_remap_key_ambiguous(self):
        directkeys.remap_key('A', 'b')
        self.do(d_a+d_b, d_b+d_b)
        self.do([make_event(KEY_DOWN, 'A', -1)], d_b)
    def test_remap_key_multiple(self):
        mapped = directkeys.remap_key('a', 'shift+b')
    ...
    def test_stash_state(self):
        self.do(d_a+d_shift)
        self.assertEqual(sorted(directkeys.stash_state()), [1, 5])
        self.do([], u_a+u_shift)
    def test_restore_state(self):
        self.do(d_b)
        directkeys.restore_state([1, 5])
        self.do([], u_b+d_a+d_shift)
    def test_restore_modifieres(self):
        self.do(d_b)
        directkeys.restore_modifiers([1, 5])
        self.do([], u_b+d_shift)
    ...
    #    self.do(d_shift)
    #    directkeys.write('a', restore_state_after=False, exact=False)
    ...
    def test_write_multiple(self):
        last_time = time.time()
        directkeys.write('ab', delay=0.01, exact=False)
        self.do([], d_a+u_a+d_b+u_b)
    ...
    def test_start_stop_recording(self):
        ...
        self.do(d_a+u_a)
        self.assertEqual(directkeys.stop_recording(), d_a+u_a)
    def test_stop_recording_error(self):
        with self.assertRaises(ValueError):
            directkeys.stop_recording()

    def test_record(self):
        queue = directkeys._queue.Queue()
        def process():
            queue.put(directkeys.record('space', suppress=True))
    ...
    def test_get_typed_strings_simple(self):
        ...
        self.assertEqual(list(directkeys.get_typed_strings(events)), ['aA ', 'a'])
    def test_get_typed_strings_backspace(self):
        events = du_a+du_b+du_backspace
        self.assertEqual(list(directkeys.get_typed_strings(events)), ['a'])
        events = du_backspace+du_a+du_b
        self.assertEqual(list(directkeys.get_typed_strings(events)), ['ab'])
    def test_get_typed_strings_shift(self):
        events = d_shift+du_a+du_b+u_shift+du_space+du_ctrl+du_a
        self.assertEqual(list(directkeys.get_typed_strings(events)), ['AB ', 'a'])
    def test_get_typed_strings_all(self):
        ...
        self.assertEqual(list(directkeys.get_typed_strings(events)), ['aAb ', 'A'])

    def test_get_hotkey_name_simple(self):
        self.assertEqual(directkeys.get_hotkey_name(['a']), 'a')
    def test_get_hotkey_name_modifiers(self):
        self.assertEqual(directkeys.get_hotkey_name(['a', 'shift', 'ctrl']), 'ctrl+shift+a')
    def test_get_hotkey_name_normalize(self):
        self.assertEqual(directkeys.get_hotkey_name(['SHIFT', 'left ctrl']), 'ctrl+shift')
    def test_get_hotkey_name_plus(self):
        self.assertEqual(directkeys.get_hotkey_name(['+']), 'plus')
    def test_get_hotkey_name_duplicated(self):
        self.assertEqual(directkeys.get_hotkey_name(['+', 'plus']), 'plus')
    def test_get_hotkey_name_full(self):
        self.assertEqual(directkeys.get_hotkey_name(['+', 'left ctrl', 'shift', 'WIN', 'right alt']), 'ctrl+alt+shift+windows+plus')
    def test_get_hotkey_name_multiple(self):
        self.assertEqual(directkeys.get_hotkey_name(['ctrl', 'b', '!', 'a']), 'ctrl+!+a+b')
    def test_get_hotkey_name_from_pressed(self):
        self.do(du_c+d_ctrl+d_a+d_b)
        self.assertEqual(directkeys.get_hotkey_name(), 'ctrl+a+b')

    def test_read_hotkey(self):
        queue = directkeys._queue.Queue()
        def process():
            queue.put(directkeys.read_hotkey())
        from threading import Thread
        t = Thread(target=process)
        t.daemon = True
        t.start()
    ...
    def test_read_event(self):
        queue = directkeys._queue.Queue()
        def process():
            queue.put(directkeys.read_event(suppress=True))
        from threading import Thread
        t = Thread(target=process)
        t.daemon = True
        t.start()
    ...
    def test_read_key(self):
        queue = directkeys._queue.Queue()
        def process():
            queue.put(directkeys.read_key(suppress=True))
        from threading import Thread
        t = Thread(target=process)
        t.daemon = True
        t.start()
    ...
    def test_wait_until_success(self):
        queue = directkeys._queue.Queue()
        def process():
            queue.put(directkeys.wait(queue.get(timeout=0.5), suppress=True) or True)
        from threading import Thread
        t = Thread(target=process)
        t.daemon = True
        t.start()
    ...
    def test_wait_until_fail(self):
        def process():
            directkeys.wait('a', suppress=True)
            self.fail()
        from threading import Thread
        t = Thread(target=process)
        t.daemon = True # Yep, we are letting this thread loose.
    ...
    def test_add_hotkey_single_step_suppress_allow(self):
        directkeys.add_hotkey('a', lambda: trigger() or True, suppress=True)
        self.do(d_a, triggered_event+d_a)
    def test_add_hotkey_single_step_suppress_args_allow(self):
        arg = object()
        directkeys.add_hotkey('a', lambda a: self.assertIs(a, arg) or trigger() or True, args=(arg,), suppress=True)
        self.do(d_a, triggered_event+d_a)
    ...
    def test_add_hotkey_single_step_suppress_removed(self):
        ...
        self.do(d_a, d_a)
    def test_add_hotkey_single_step_suppress_removed(self):
        directkeys.remove_hotkey(directkeys.add_hotkey('ctrl+a', trigger, suppress=True))
        self.do(d_ctrl+d_a, d_ctrl+d_a)
        self.assertEqual(directkeys._listener.filtered_modifiers[dummy_keys['left ctrl'][0][0]], 0)
    def test_remove_hotkey_internal(self):
        remove = directkeys.add_hotkey('shift+a', trigger, suppress=True)
        self.assertTrue(all(directkeys._listener.blocking_hotkeys.values()))
        self.assertTrue(all(directkeys._listener.filtered_modifiers.values()))
        self.assertNotEqual(directkeys._hotkeys, {})
        remove()
        self.assertTrue(not any(directkeys._listener.filtered_modifiers.values()))
        self.assertTrue(not any(directkeys._listener.blocking_hotkeys.values()))
        self.assertEqual(directkeys._hotkeys, {})
    def test_remove_hotkey_internal_multistep_start(self):
        remove = directkeys.add_hotkey('shift+a, b', trigger, suppress=True)
        self.assertTrue(all(directkeys._listener.blocking_hotkeys.values()))
        self.assertTrue(all(directkeys._listener.filtered_modifiers.values()))
        self.assertNotEqual(directkeys._hotkeys, {})
        remove()
    ...
    def test_remove_hotkey_internal_multistep_end(self):
        ...
        self.assertTrue(any(directkeys._listener.blocking_hotkeys.values()))
        self.assertTrue(not any(directkeys._listener.filtered_modifiers.values()))
        self.assertNotEqual(directkeys._hotkeys, {})
        remove()
        self.assertTrue(not any(directkeys._listener.filtered_modifiers.values()))
    ...
    def test_add_hotkey_single_step_suppress_with_modifiers_repeated(self):
        directkeys.add_hotkey('ctrl+a', trigger, suppress=True)
        self.do(d_ctrl+du_a+du_b+du_a, triggered_event+d_ctrl+du_b+triggered_event)
    def test_add_hotkey_single_step_suppress_with_modifiers_release(self):
        directkeys.add_hotkey('ctrl+a', trigger, suppress=True, trigger_on_release=True)
        self.do(d_ctrl+du_a+du_b+du_a, triggered_event+d_ctrl+du_b+triggered_event)
    ...
    def test_add_hotkey_single_step_timeout(self):
        directkeys.add_hotkey('a', trigger, timeout=1, suppress=True)
        self.do(du_a, triggered_event)
    def test_add_hotkey_multi_step_first_timeout(self):
        directkeys.add_hotkey('a, b', trigger, timeout=0.01, suppress=True)
        time.sleep(0.03)
    ...
    def test_add_hotkey_single_step_nosuppress_with_modifiers_out_of_order(self):
        queue = directkeys._queue.Queue()
        directkeys.add_hotkey('ctrl+shift+a', lambda: queue.put(True), suppress=False)
    ...
    def test_remap_hotkey_release_incomplete(self):
        directkeys.remap_hotkey('a', 'b', trigger_on_release=True)
        self.do(d_a, [])
    ...
    def test_parse_hotkey_combinations_scan_code(self):
        self.assertEqual(directkeys.parse_hotkey_combinations(30), (((30,),),))
    def test_parse_hotkey_combinations_single(self):
        self.assertEqual(directkeys.parse_hotkey_combinations('a'), (((1,),),))
    def test_parse_hotkey_combinations_single_modifier(self):
        self.assertEqual(directkeys.parse_hotkey_combinations('shift+a'), (((1, 5), (1, 6)),))
    def test_parse_hotkey_combinations_single_modifiers(self):
        self.assertEqual(directkeys.parse_hotkey_combinations('shift+ctrl+a'), (((1, 5, 7), (1, 6, 7)),))
    def test_parse_hotkey_combinations_multi(self):
        self.assertEqual(directkeys.parse_hotkey_combinations('a, b'), (((1,),), ((2,),)))
    def test_parse_hotkey_combinations_multi_modifier(self):
        self.assertEqual(directkeys.parse_hotkey_combinations('shift+a, b'), (((1, 5), (1, 6)), ((2,),)))
    def test_parse_hotkey_combinations_list_list(self):
        self.assertEqual(directkeys.parse_hotkey_combinations(directkeys.parse_hotkey_combinations('a, b')), directkeys.parse_hotkey_combinations('a, b'))
    ...
    def test_add_hotkey_multistep_suppress_incomplete(self):
        ...
        self.assertEqual(len(directkeys._listener.blocking_hotkeys[(2,)]), 1)
    def test_add_hotkey_multistep_suppress_incomplete(self):
        directkeys.add_hotkey('a, b', trigger, suppress=True)
        self.do(du_a+du_b, triggered_event)
    def test_add_hotkey_multistep_suppress_modifier(self):
        directkeys.add_hotkey('shift+a, b', trigger, suppress=True)
    ...
    def test_add_hotkey_multistep_suppress_three_steps(self):
        directkeys.add_hotkey('a, b, c', trigger, suppress=True)
        self.do(du_a+du_b+du_c, triggered_event)
    def test_add_hotkey_multistep_suppress_repeated_prefix(self):
        directkeys.add_hotkey('a, a, c', trigger, suppress=True, trigger_on_release=True)
    ...
    def test_add_hotkey_multistep_suppress_repeated_key(self):
        ...
        self.assertEqual(directkeys._listener.blocking_hotkeys[(2,)], [])
        self.assertEqual(len(directkeys._listener.blocking_hotkeys[(1,)]), 1)
    ...
    def test_add_word_listener_success(self):
        ...
        def free():
            queue.put(1)
        directkeys.add_word_listener('abc', free)
    ...
    def test_add_word_listener_remove(self):
        ...
        def free():
            queue.put(1)
        directkeys.add_word_listener('abc', free)
        directkeys.remove_word_listener('abc')
        self.do(du_a+du_b+du_c+du_space)
        with self.assertRaises(directkeys._queue.Empty):
            queue.get(timeout=0.01)
    def test_add_word_listener_suffix_success(self):
        ...
        def free():
            queue.put(1)
        directkeys.add_word_listener('abc', free, match_suffix=True)
    ...
    #    directkeys.add_abbreviation('abc', 'aaa')
    #    self.do(du_a+du_b+du_c+du_space, [])
