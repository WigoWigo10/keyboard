...
directkeys._os_keyboard.press = lambda scan_code: send_instant_event(
    make_event(KEY_DOWN, None, scan_code)
)
directkeys._os_keyboard.release = lambda scan_code: send_instant_event(
    make_event(KEY_UP, None, scan_code)
)
directkeys._os_keyboard.type_unicode = lambda char: output_events.append(
...
class TestKeyboard(unittest.TestCase):
    ...
    def test_key_to_scan_code_from_normalized(self):
        ...
        self.assertEqual(directkeys.key_to_scan_codes("SHIFT"), (5, 6))
        self.assertEqual(
            directkeys.key_to_scan_codes("ctrl"), directkeys.key_to_scan_codes("CONTROL")
        )
    ...
    def test_parse_hotkey_keys(self):
        self.assertEqual(
            directkeys.parse_hotkey("left shift + a"),
            (
                (
                    (5,),
                    (1,),
    ...
    def test_parse_hotkey_example(self):
        ...
        self.assertEqual(
            directkeys.parse_hotkey("alt+shift+a, alt+b, c"),
    ...
    def test_get_hotkey_name_full(self):
        self.assertEqual(
            directkeys.get_hotkey_name(["+", "left ctrl", "shift", "WIN", "right alt"]),
    ...
    def test_add_hotkey_single_step_suppress_with_modifier_superset_release(self):
        directkeys.add_hotkey("ctrl+a", trigger, suppress=True, trigger_on_release=True)
        self.do(
            d_ctrl + d_shift + du_a + u_shift + u_ctrl, d_ctrl + d_shift + du_a + u_shift + u_ctrl
        )
    ...
    def test_parse_hotkey_combinations_single_modifiers(self):
        self.assertEqual(
            directkeys.parse_hotkey_combinations("shift+ctrl+a"), (((1, 5, 7), (1, 6, 7)),)
        )
    ...
    def test_parse_hotkey_combinations_multi_modifier(self):
        self.assertEqual(
            directkeys.parse_hotkey_combinations("shift+a, b"), (((1, 5), (1, 6)), ((2,),))
        )

    def test_parse_hotkey_combinations_list_list(self):
        self.assertEqual(
            directkeys.parse_hotkey_combinations(directkeys.parse_hotkey_combinations("a, b")),
            directkeys.parse_hotkey_combinations("a, b"),
        )
